pipeline{
    ...
    stages {
        stage('Dev - Deployment'){
            steps {
                sh 'sudo scp -i /var/lib/jenkins/workspace/Jenkin.pem -r ../Mental-Health-Assistance ubuntu@ec2-3-144-180-19.us-east-2.compute.amazonaws.com:~'
                sh 'sudo ssh -i /var/lib/jenkins/workspace/Jenkin.pem ubuntu@ec2-3-144-180-19.us-east-2.compute.amazonaws.com "cd ~/Mental-Health-Assistance/ && sudo chmod -R 777 ./Jenkins/ && ./Jenkins/app_setup.sh > deployment.log  && ./Jenkins/app_start.sh > deployment.log && ./Jenkins/env_setup.sh > deployment.log"'
            }
            post {
                always {
